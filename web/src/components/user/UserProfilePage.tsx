...
export default function UserProfilePage({ targetUser, thisPage }: { targetUser: UserData, thisPage: string }) {
  ...
  const [nowUser, setNowUser] = useState<UserData | null>(null);


  const [isUserImgReady, setIsUserImgReady] = useState<boolean>(false);
  const [isGetImgsReady, setIsGetImgsReady] = useState<boolean>(false);
  ...
  const getUserItems = useCallback(
    async (pageNumber: number) => {
      ...
      try {
        ...
        // flushSync는 React의 상태 업데이트를 즉시 동기적으로 처리하는 함수
        // 상태 업데이트를 즉시 반영한다.
        // 다음 렌더링 사이클을 기다리지 않고 바로 렌더링함.
        flushSync(() => {
  ...
  }, [getUserItems, page, nowUser]); // 마운트가 된다는 것은 dom에 추가되어 렌더링이 된다는 것
  ...
  const uploadCroppedImage = async () => {
    if (!imageSrc || !croppedAreaPixels || !nowUser?.userId) return;
    const croppedImageBlob = await getCroppedImg(imageSrc, croppedAreaPixels);

    // 이 blob을 FormData에 담아 업로드하면 됨
    const formData = new FormData();
    formData.append('userId', nowUser?.userId);
  ...
  return (
    nowUser && isGetImgsReady && isUserImgReady && (
